// Dijkstra - O(|V|^2)
// 이미 최단 경로가 확정된 꼭짓점 집합 S를 관리함으로써 최단 거리 갱신
// 현재 최단 거리이고 사용되지 않은 vertex를 대상으로 완화해서 최단 거리 갱신
#include <iostream>
#include <vector>
using namespace std;
...
int main() {
    ...
    // 다익스트라 알고리즘
    vector<bool> used(N, false); // 집합 S에 포함 여부 판정
    vector<long long> dist(N, INF); // 시작점 s에서부터 각 vertex까지의 거리
    dist[s] = 0;
